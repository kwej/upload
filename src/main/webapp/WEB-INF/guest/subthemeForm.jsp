...
<script type="text/javascript">
	...
	function doLogin(name) {
		...
		location.href="login.gt?destination=subtheme&name="+name;
	}
...
<div class="container">
    <div class="row justify-content-center align-items-center">
      <div class="col-lg-9 text-center mt-5">
        <h1 class="heading" data-aos="fade-up">${subthemeName}</h1>
...
<div class="section section-properties">
      <div class="container">
        <div class="row">
        	...
        	<c:forEach var="sub" items="${subThemeList }">
        	
        	<%String imgurl = request.getContextPath()+"/resources/house_images/";  %>
            <c:set var="image" value="${fn:split(sub.photo ,',')}"></c:set>
        	
        	<!-- .item 시작 -->
			<div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
            <div class="property-item mb-30">
              <a href="detail.gt?house_no=${sub.house_no }" class="img">
                <c:forEach var="img" items="${image}" varStatus="status">
               <c:if test="${status.count eq 1}">
                  <img src="<%=imgurl %>${img }" alt="Image" class="img-fluid"/>
               </c:if>
                </c:forEach>
              ...
              <c:choose>
	              <c:when test="${guestInfo != null}"> <%-- 로그인 했을때 : 자신이 클릭한 하트, 클릭하지 않은 하트 나오도록 --%>
	              	<c:if test="${fn:contains(houseNos,sub.house_no) == false}">
	              		<a href="javascript:clickEmptyHeart(${sub.house_no})" id="heart_click_${sub.house_no}">
		                  <img src="resources/images/heart-regular-24.png" class="empty heart_img" /> <!-- 빈하트 -->
		                </a>
	              	</c:if>
	              	
	              	<c:if test="${fn:contains(houseNos,sub.house_no)}">
	              		<a href="javascript:clickFullHeart(${sub.house_no})" id="heart_click_${sub.house_no}">
	              ...
	              <%-- 로그인 안했을때 : 모든 하트 버튼 클릭 X --%>
			  	  <c:otherwise>
			  	  	<a href="javascript:doLogin('${sub.sname}')">
              ...
              <div class="property-content">
                <div class="price mb-2"><span>￦<fmt:formatNumber value="${sub.price }" pattern="#,###"/></span></div>
                <div>
                  <span class="d-block mb-2 text-black-50"
                    >${sub.address }/${sub.detailed }</span
                  >
                  <span class="city d-block mb-3">${sub.name }</span>

                  <a
                    href="detail.gt?house_no=${sub.house_no }"
