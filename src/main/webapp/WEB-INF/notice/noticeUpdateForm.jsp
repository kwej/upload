...
<html lang="en">
  ...
  <script type="text/javascript">
	function cateChange(){
		...
		nno = myform.nno.value;
		
		location.href="updateCate.nt?c_selValue="+c_selValue+"&nno="+nno;
	}
	...
	function coupone_Change(){
		...
		location.href="updateCoupon.nt?cou_selValue="+cou_selValue+"&c_selValue="+c_selValue+"&nno="+nno;
	}
      ...
      <div class="container">
        <div class="text-center pb-2">
          <p class="section-title px-5">
            <span class="px-2">[*오타 없나 유의 하기/카테고리 선택하기]</span>
          </p>
        </div>
        <div class="row">
          <div class="col-lg-12 mb-9">
            <div class="contact-form">
              <form:form commandName="notice" action="nupdate.nt" method="post" encType="multipart/form-data" name="myform">
              ...
              <div id="success"></div>
               		 ...
               		 <select class="form-select" aria-label="Default select example" id="cate" name="cate" onChange="cateChange()">
						   <option value="">카테고리를 선택하세요</option>
						   		<c:forEach var="i" items="${cate }">
                           			<option value="${i}" <c:if test="${c_selValue eq i || notice.cate eq i}">selected</c:if>>${i }</option>
                           		</c:forEach>
					</select>
				...
				<div class="mb-3">
					<select class="form-select" aria-label="Default select example" id="coupon" name="coupon" onChange="coupone_Change()">
						...
						<c:forEach var="i" items="${lists}">
							<option value="${i.name}" <c:if test="${cou_selValue eq i.name ||notice.coupon eq i.name}">selected</c:if>>${i.name}</option>
						</c:forEach>
					</select>
      		  </div>
      		  </div>
      		  ...
      		  <div class="control-group">
                  <input type="text" class="form-control" id="name" placeholder="제목 입력" name="title" value="${ notice.title}${title}"/>
                  <p class="help-block text-danger"></p>
      		 ...
      		 <c:if test="${c_selValue eq '쿠폰' }">
                <div class="control-group">
                  ...
                  >[${code }]  &nbsp; 해당 쿠폰코드를 복사해서 쿠폰등록하기를 눌러 등록해주세요</textarea>
                  <p class="help-block text-danger"></p>
                </div>
           	</c:if>
